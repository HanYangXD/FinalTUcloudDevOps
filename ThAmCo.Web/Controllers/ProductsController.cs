...
namespace ThAmCo.Web.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public async Task<IActionResult> Index(string productBrand, string productCategory, string searchString)
        {
            IQueryable<string> brandQuery = from m in _context.Products orderby m.Brand.Name select m.Brand.Name;
            IQueryable<string> categoryQuery = from m in _context.Products orderby m.Category.Name select m.Category.Name;

            var products = from m in _context.Products.Include(p => p.Brand).Include(p => p.Category) select m;
            ...
            {
                products = products.Where(s => s.Category.Name == productCategory);
            }

            var productBrandVM = new ProductBrandViewModel
            {
